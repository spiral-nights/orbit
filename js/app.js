...
function setupEventListeners() {
    ...
    themeToggle.addEventListener('click', () => {
        ...
        // Reload app with new theme if active
        if (isAppActive) {
            reloadCurrentApp();
        }
    });
...
// Reload current app (used when theme changes)
function reloadCurrentApp() {
    try {
        const currentSrc = appFrame.src;
        if (currentSrc && currentSrc.includes('/apps/')) {
            // Update the dark mode parameter
...
function launchApp(id) {
    ...
    // Get the app URL from storage helper (includes dark mode param if needed)
    const isDark = document.body.classList.contains('dark');
    const appUrl = Storage.getAppUrl(app, isDark);

    // Navigate iframe to app URL - Service Worker will serve content from IndexedDB
    appFrame.src = appUrl;
}

function closeApp(openMenu = false) {
    // Navigate iframe back to blank
    appFrame.src = 'about:blank';
